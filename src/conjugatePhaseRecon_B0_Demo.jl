...
## GPU Compatible Conjugate Phase Reconstruction with B₀ Correction (0.5s per recon :) ) (REQUIRES 8GB GPU Memory or Higher)
...
showReconstructedImage("Reference Signal Distribution", I_mage, imShape, false)
...
parameters[:correctionMap] = 1im .* quadraticFieldmap(N,M,125.0*pi)
...
b0_map = CuVector(Float32.(vec(quadraticFieldmap(N,M,125.0*pi)))) # -> radians off resonance, divide by 2 pi to get Hz
